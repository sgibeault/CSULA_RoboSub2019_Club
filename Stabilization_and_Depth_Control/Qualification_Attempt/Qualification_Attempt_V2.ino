 ...
 *  061419: Sidra - Removed manual control code, added pre-set path for sub to take
 *  061719: Sidra - Changed qualification run code to be based on IMU angle
 *  
 */
...
float desired_x_angle = 0; //Setpoint x angle
float desired_y_angle = 0;
float desired_z_angle = 0;
float desired_depth = 0; //Setpoint depth
...
if (millis() < 10000){
  ...
  thruster_backright.writeMicroseconds(pwmBackRight); 
  thruster_backleft.writeMicroseconds(pwmBackLeft);
  thruster_sideright.writeMicroseconds(pwmSideRight+120);
  thruster_sideleft.writeMicroseconds(pwmSideLeft+120);
  
}

else if (millis() < 54000){
